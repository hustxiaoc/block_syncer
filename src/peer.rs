...
impl Peer {
    ...
    pub async fn connect(&mut self) -> Result<(), Box<dyn Error>> {
        ...
        loop {
            tokio::select! {
                ...
                _ = self.read_message(&mut reader) => {
    ...
    async fn read_message(
        &self,
        mut reader: &mut ReadHalf<TcpStream>, 
        ...
        if command == "block" {
            ...
            let block_hash = block_header.hash().encode();
            {
                ...
                if (*lock).contains(&block_hash) {
                    ...
                    drain_buf!(reader, payload_size, 1024);
                    return Ok(());
                }
            }
        ...
        match message {
            ...
            Ok(Message::GetBlocks(_)) => {
                let error = format!("We don't provide such service byebye! node with version");
                return Err(error.into());
            },
            ...
            Ok(Message::Headers(headers)) => {
                ...
                for header in headers {
                    if header.hash().encode() == first_block_header {
                        let error = format!("Invalid node with version");
                        return Err(error.into());
                    }
